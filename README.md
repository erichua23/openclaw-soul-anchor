...
约束写得再好，模型不服从也白搭。不同模型对 system prompt 中指令的遵循程度差异很大——有的模型在被明确禁止的情况下仍然会执行操作，有的在社工话术诱导下轻易破防。

我们做了一个专门测试模型指令服从度的基准项目：**[LLM Compliance Bench](https://github.com/erichua23/llm-compliance-bench)**，覆盖 6 个维度：
...
如果你在选型阶段，建议先跑一遍 Compliance Bench 看看目标模型的服从度，再决定约束策略的力度。
